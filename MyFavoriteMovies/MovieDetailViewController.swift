...
class MovieDetailViewController: UIViewController {
    ...
    @IBAction func toggleFavorite(sender: AnyObject) {
        ...
        let dataToSend: NSDictionary = [
            "media_type" : "movie",
            "media_id" : self.movie!.id,
            "favorite" : shouldFavorite]
        
        do {
            request.HTTPBody = try NSJSONSerialization.dataWithJSONObject(dataToSend, options: .PrettyPrinted)
        } catch {
            print(error)
        }
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            guard let succCode = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int else {
            ...
            if shouldFavorite && !(succCode == 1 || succCode == 12) {
                return
            } else if !shouldFavorite && succCode != 13 {
                return
            }
